...
int main(void){
    ...
    day = day_of_year(2023, 2, 29);
    printf("day of year: %d\n", day);
    
    month_day(2024, 367, &months, &days);
...
int day_of_year(int year, int month, int day){
    ...
    if(day > daytab[leap][month]){
        printf("Error: You've specified %d days but there are only %d days in this month", day, daytab[leap][month]);
        return -1;
    }
    for(i = 1; i < month; i++){
